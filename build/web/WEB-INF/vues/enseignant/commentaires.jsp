...
<%@ page import="model.Utilisateur, model.Notification, java.util.List, java.text.SimpleDateFormat" %>
<%
    Utilisateur userSession = (Utilisateur) session.getAttribute("utilisateur");
    List<Notification> notificationsRecues = (List<Notification>) request.getAttribute("notificationsRecues");
    Long nbNotifs = (Long) request.getAttribute("nbNotifs");
    String activeSection = (String) request.getAttribute("activeSection");
    if (activeSection == null) activeSection = "commentaires";
    String ctx = request.getContextPath();
    SimpleDateFormat sdfNotif = new SimpleDateFormat("dd/MM/yyyy HH:mm");
%>
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8"/>
    <title>Commentaires – Enseignant</title>
...
<div class="flex flex-1">
    <aside class="w-64 bg-white shadow-md flex flex-col min-h-full">
        <nav class="flex flex-col p-4 gap-2 flex-1">
            ...
            <a href="<%= ctx %>/enseignant/CommentaireServlet"
               class="flex items-center gap-3 px-4 py-3 rounded-lg font-medium <%= "commentaires".equals(activeSection) ? "bg-primary text-white" : "text-gray-700 hover:bg-gray-100 transition" %>">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <main class="flex-1 p-6">
        <div class="bg-white rounded-xl shadow p-8 mb-6">
            <h1 class="text-2xl font-bold text-primary mb-2">Commentaires</h1>
            <p class="text-gray-500 text-sm mb-4">
                Les commentaires des étudiants sur vos TPs vous sont notifiés. Consultez vos TPs pour voir et répondre aux échanges.
            </p>
            <a href="<%= ctx %>/enseignant/CorrectionTPServlet?action=list"
               class="inline-flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-lg hover:bg-blue-900 transition text-sm font-medium">
                Voir les TPs à corriger
            </a>
        </div>

        <%-- Messages reçus (admin, étudiants) : pouvoir répondre --%>
        <div class="bg-white rounded-xl shadow p-6">
            <h2 class="text-lg font-bold text-primary mb-2">Messages reçus (admin, étudiants)</h2>
            <p class="text-gray-500 text-sm mb-4">Répondez aux messages envoyés par l'admin ou les étudiants.</p>
            <% if (notificationsRecues != null && !notificationsRecues.isEmpty()) {
                int countWithExpediteur = 0;
                for (Notification n : notificationsRecues) {
                    if (n.getExpediteur() != null) countWithExpediteur++;
                }
                if (countWithExpediteur > 0) { %>
            <div class="space-y-3">
                <% for (Notification n : notificationsRecues) {
                    if (n.getExpediteur() == null) continue;
                    String replyUrl = ctx + "/enseignant/MessageServlet?destinataireId=" + n.getExpediteur().getId();
                %>
                <div class="border border-gray-100 rounded-lg p-4 hover:bg-gray-50 transition">
                    ...
                    <p class="text-sm text-gray-700"><%= n.getMessage() %></p>
                    <p class="text-xs text-gray-400 mt-1"><%= sdfNotif.format(n.getDateCreation()) %></p>
                    <a href="<%= replyUrl %>" class="inline-block mt-2 text-sm text-primary font-medium hover:underline">Répondre</a>
                </div>
                <% } %>
            </div>
            <% } else { %>
            <p class="text-gray-400 text-sm">Aucun message reçu pour l'instant.</p>
            <% }
            } else { %>
            <p class="text-gray-400 text-sm">Aucun message reçu pour l'instant.</p>
            <% } %>
