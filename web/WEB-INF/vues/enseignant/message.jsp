...
<%@ page import="model.Utilisateur, model.Etudiant, model.Notification, java.util.List, java.util.Map, java.text.SimpleDateFormat, util.HtmlUtil" %>
...
<head>
    <meta charset="UTF-8"/>
    <title>Envoyer un message – Enseignant</title>
    ...
    <div class="flex items-center gap-4">
        ...
        <button type="button" onclick="toggleProfilePanel()" class="flex items-center gap-2">
            <div class="w-9 h-9 bg-blue-400 rounded-full flex items-center justify-center font-bold text-white text-sm">
                <%= userSession != null && userSession.getPrenom() != null && userSession.getNom() != null ? HtmlUtil.escape(String.valueOf(userSession.getPrenom().charAt(0)) + String.valueOf(userSession.getNom().charAt(0))) : "EN" %>
            </div>
            <span class="text-sm font-medium hidden md:block"><%= userSession != null ? HtmlUtil.escape(userSession.getNomComplet()) : "Enseignant" %></span>
        </button>
    </div>
</header>
...
<div class="flex flex-1">
    <aside class="w-64 bg-white shadow-md flex flex-col min-h-full">
        <nav class="flex flex-col p-4 gap-2 flex-1">
            <a href="<%= ctx %>/enseignant/DashboardServlet" class="flex items-center gap-3 px-4 py-3 rounded-lg font-medium transition <%= "dashboard".equals(activeSection) ? "bg-primary text-white" : "text-gray-700 hover:bg-gray-100" %>">
            ...
            <a href="<%= ctx %>/enseignant/CorrectionTPServlet?action=list" class="flex items-center gap-3 px-4 py-3 rounded-lg font-medium transition <%= "tps".equals(activeSection) ? "bg-primary text-white" : "text-gray-700 hover:bg-gray-100" %>">
            ...
            <a href="<%= ctx %>/enseignant/CommentaireServlet" class="flex items-center gap-3 px-4 py-3 rounded-lg font-medium transition <%= "commentaires".equals(activeSection) ? "bg-primary text-white" : "text-gray-700 hover:bg-gray-100" %>">
            ...
            <a href="<%= ctx %>/enseignant/AbsenceServlet" class="flex items-center gap-3 px-4 py-3 rounded-lg font-medium transition <%= "absences".equals(activeSection) ? "bg-primary text-white" : "text-gray-700 hover:bg-gray-100" %>">
            ...
            <a href="<%= ctx %>/enseignant/MessageServlet" class="flex items-center gap-3 px-4 py-3 rounded-lg font-medium <%= "messages".equals(activeSection) ? "bg-primary text-white" : "text-gray-700 hover:bg-gray-100 transition" %>">
    ...
    <main class="flex-1 p-6">
        <h2 class="text-2xl font-bold text-primary mb-2">Envoyer un message à un étudiant</h2>
        <p class="text-gray-500 text-sm mb-6">Les étudiants listés ont déposé au moins un TP dans vos modules. Le message apparaîtra dans leurs notifications.</p>
        <% if (sent) { %>
        ...
        <div class="bg-white rounded-xl shadow p-6 max-w-xl">
            <h3 class="text-lg font-semibold text-primary mb-3">Conversation avec <%= HtmlUtil.escape(replyToUser.getNomComplet()) %></h3>
        ...
        <div class="bg-white rounded-xl shadow p-8 text-center text-gray-500">
            Aucun étudiant dans vos modules pour l'instant. Les étudiants apparaîtront ici après avoir déposé un TP.
        </div>
        <% } else { %>
        <div class="bg-white rounded-xl shadow p-6 max-w-xl">
            <div class="mb-4">
                ...
                <select id="selectEtudiant" class="border border-gray-300 rounded-lg px-3 py-2 w-full text-sm">
                    ...
                    %>
                    <option value="<%= e.getId() %>"<%= (preselectedDestinataireId != null && preselectedDestinataireId.equals(e.getId())) ? " selected" : "" %>><%= HtmlUtil.escape(e.getNomComplet()) %><%= lastDate != null ? " – Dernier: " + sdfTime.format(lastDate) : "" %><%= unreadInt > 0 ? " (" + unreadInt + " non lu(s))" : "" %></option>
                    <% } %>
                </select>
            </div>
