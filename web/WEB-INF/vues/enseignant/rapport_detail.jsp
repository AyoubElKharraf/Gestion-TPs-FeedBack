...
<%@ page import="model.Utilisateur, model.Rapport, java.text.SimpleDateFormat" %>
<%
...
<head>
    <meta charset="UTF-8"/>
    <title>Détail rapport – Enseignant</title>
...
<header class="bg-primary text-white px-6 py-4 flex items-center justify-between shadow-md">
    <div class="flex items-center gap-3">
        <a href="<%= ctx %>/enseignant/RapportServlet" class="p-2 rounded-full hover:bg-blue-900 transition">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
            </svg>
        </a>
        <span class="text-xl font-bold truncate"><%= rapport != null ? rapport.getTitre() : "Rapport" %></span>
    </div>
...
<div class="flex flex-1">
    ...
    <main class="flex-1 p-6 max-w-2xl">
        <a href="<%= ctx %>/enseignant/RapportServlet" class="inline-flex items-center gap-2 text-sm text-primary hover:underline mb-4">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            Retour aux rapports
        </a>
        <% if (rapport == null) { %>
        <div class="bg-red-50 text-red-600 p-4 rounded-xl">Rapport introuvable.</div>
        <% } else { %>
        <div class="bg-white rounded-xl shadow border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-primary/5 to-blue-50">
                <h1 class="text-xl font-bold text-primary"><%= rapport.getTitre() %></h1>
                <p class="text-sm text-gray-500 mt-1">Support de cours</p>
            </div>
            <div class="p-6 space-y-4">
                <div>
                    <span class="text-xs font-medium text-gray-500 uppercase">Module</span>
                    <p class="text-gray-800 font-medium"><%= rapport.getModule() != null ? rapport.getModule().getNom() : "–" %></p>
                </div>
                <div>
                    <span class="text-xs font-medium text-gray-500 uppercase">Date de dépôt</span>
                    <p class="text-gray-800"><%= rapport.getDateCreation() != null ? sdf.format(rapport.getDateCreation()) : "–" %></p>
                </div>
                <% if (rapport.getDateLimite() != null) { %>
                <div>
                    <span class="text-xs font-medium text-gray-500 uppercase">Date limite (dépôt TP étudiant)</span>
                    <p class="text-gray-800"><%= sdf.format(rapport.getDateLimite()) %></p>
                </div>
                <% } %>
                <div>
                    <span class="text-xs font-medium text-gray-500 uppercase">Fichier</span>
                    <p class="text-gray-800"><%= rapport.getFileName() != null ? rapport.getFileName() : "–" %></p>
                </div>
                <div class="flex flex-wrap gap-3 pt-2">
                    <a href="<%= ctx %>/RapportDownloadServlet?id=<%= rapport.getId() %>" target="_blank"
                    ...
                    <a href="<%= ctx %>/enseignant/RapportServlet?action=delete&id=<%= rapport.getId() %>"
...
function toggleNotifications() {
    ...
    if (!panel.classList.contains('hidden') && document.getElementById('notifList').innerHTML.indexOf('Chargement') !== -1) {
